...
class NeuralSentimentClassifier(SentimentClassifier):
    ...
    def preprocess_sentence(self, sentence: List[str]) -> torch.Tensor:
        """
        ...
        return torch.tensor([i if i != -1 else idx("UNK") for i in idxs])

    def predict(self, ex_words: List[str]) -> int:
        """
        @see SentimentClassifier#predict
        """
        log_probs = self.ffnn.forward(self.preprocess_sentence(ex_words))
        return torch.argmax(log_probs)

    def train(self, ex_words: List[str], label: int) -> float:
        """
        NN train step
        ...
        x = self.preprocess_sentence(ex_words)
        ...
        label_onehot.scatter_(0, torch.from_numpy(np.asarray(label, dtype=np.int64)), 1)
...
class SentimentNN(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        """
        embedded = torch.mean(self.embedding(x), 0)
...
def train_deep_averaging_network(args, train_exs: List[SentimentExample], dev_exs: List[SentimentExample], word_embeddings: WordEmbeddings) -> NeuralSentimentClassifier:
    ...
    for epoch in range(0, args.num_epochs):
        ...
        for idx in ex_indices:
            train_ex = train_exs[idx]
            loss = classifier.train(train_ex.words, train_ex.label)
            total_loss += loss
...
if __name__=="__main__":
    ...
    log_probs = classifier.ffnn.forward(classifier.preprocess_sentence(sentence))
